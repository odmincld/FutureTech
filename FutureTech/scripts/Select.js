...
class Select extends BaseComponent {
  ...
  fixDropdownPosition() {
    ...
    const isButtonOnTheViewportSiede = buttonCenterX < halfViewportX;

    this.dropdownElement.classList.toggle(
      this.stateClasses.isOnTheLeftSide,
      isButtonOnTheViewportSiede
    );
  ...
  onClick = (event) => {
    ...
    if (isOptionClick) {
      ...
      this.state.currentOptionIndex = [...this.optionElement].findIndex(
  ...
  onEndKeyDown = () => {
    ...
    if (this.state.currentOptionIndex !== this.optionElement.length - 1) {
      this.state.currentOptionIndex = this.optionElement.length - 1;
    }
  };
